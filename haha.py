...
def projection(rho):
	...
	rho = rho - Constant(0.1187)
	rho = interpolate(rho, V)
	rho = interpolate((rho - assemble(rho * dx)/omega), V)
...
def FormObjectiveGradient(tao, x, G):
	...
	# newdJdrho = assemble((dJdrho - assemble(dJdrho * dx)/omega))
	newdJdrho = interpolate((dJdrho - assemble(dJdrho * dx)/omega), V)
	# print(assemble(newdJdrho * dx))
...
# FormEqualityConstraints(tao,x,x)
